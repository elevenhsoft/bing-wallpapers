...
impl Application for Images {
    ...
    fn title(&self) -> String {
        let win_title = format!(
            "{0}{1}",
            "Bing Wallpapers: ", self.json.images[self.current as usize].copyright
        );
    ...
    fn view(&self) -> Element<Buttons> {
        ...
        let img = image::Handle::from_memory(
            Images::fetch_image(Images::image_url(curr_endpoint.to_string())).unwrap(),
        );

        column![
            row![image::viewer(img).width(Length::Fixed(800.0))],
            row![column![
                row![text(&self.json.images[self.current as usize].copyright)]
                    .padding(20)
                ...
                row![
                    ...
                    .padding(3),
                    column![button(
                        text("Next >").horizontal_alignment(iced::alignment::Horizontal::Center)
                    )
                ...
                row![column![button(
                    text("Download and save to Pictures")
                        .horizontal_alignment(iced::alignment::Horizontal::Center)
                )
